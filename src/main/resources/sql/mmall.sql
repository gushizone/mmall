...
-- 数据库初始化脚本
-- 表关系
-- 1. 用户注册/登录（mmall_user）
-- 2. 搜索查看产品（mmall_category->mmall_product）
-- 3. 加入购入车（mmall_product）
-- 4. 订单确认页，收货配置（mmall_shippiing）
-- 5. 生成订单,总订单(mmall_order)可对多个子订单(mmall_order_item)
-- 6. 付款订单信息（mmall_pay_info）
